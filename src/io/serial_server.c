...
void serial_server_attach(void) {
    LOG_INFO("Awaiting serial connection on port %s...\n", SERIAL_PORT_NO);
    ...
    /* hints.ai_protocol = htonl(SERIAL_PORT_NO); */
    hints.ai_flags = AI_PASSIVE;

    getaddrinfo("localhost", SERIAL_PORT_NO, &hints, &addrinfo);

    int listenfd = socket(addrinfo->ai_family, addrinfo->ai_socktype,
                          addrinfo->ai_protocol);

    bind(listenfd, addrinfo->ai_addr, addrinfo->ai_addrlen);
